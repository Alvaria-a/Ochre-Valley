...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if("headshot")
					...
					var/new_headshot_link = trim(tgui_input_text(user, "Input the headshot link (https, hosts: gyazo, lensdump, imgbox, catbox):", "Headshot", headshot_link, max_length = MAX_MESSAGE_LEN, encode = FALSE), MAX_MESSAGE_LEN) //OV EDIT - Discord Removed
				...
				if("lich_headshot")
					...
					to_chat(user, "<span class='notice'>If the photo doesn't show up properly in-game, ensure that it's a direct image link that opens properly in a browser.</span>")
					to_chat(user, "<span class='notice'>Keep in mind that the photo will be downsized to 325x325 pixels, so the more square the photo, the better it will look.</span>")
					var/new_lich_headshot_link = trim(tgui_input_text(user, "Input the Lich headshot link (https, hosts: gyazo, lensdump, imgbox, catbox):", "Lich Headshot", lich_headshot_link, max_length = MAX_MESSAGE_LEN, encode = FALSE), MAX_MESSAGE_LEN) //OV EDIT - Discord Removed
				...
				if("vampire_headshot")
					...
					to_chat(user, "<span class='notice'>If the photo doesn't show up properly in-game, ensure that it's a direct image link that opens properly in a browser.</span>")
					to_chat(user, "<span class='notice'>Keep in mind that the photo will be downsized to 325x325 pixels, so the more square the photo, the better it will look.</span>")
					var/new_vampire_headshot_link = trim(tgui_input_text(user, "Input the vampire headshot link (https, hosts: gyazo, lensdump, imgbox, catbox):", "Vampire Headshot", vampire_headshot_link, max_length = MAX_MESSAGE_LEN, encode = FALSE), MAX_MESSAGE_LEN) //OV EDIT - Discord Removed
				...
				if("img_gallery")
					...
					var/new_galleryimg = trim(tgui_input_text(user, "Input the image link (https, hosts: gyazo, lensdump, imgbox, catbox):", "Gallery Image", max_length = MAX_MESSAGE_LEN, encode = FALSE), MAX_MESSAGE_LEN) //OV EDIT - Discord Removed
				...
				if("nsfw_img_gallery")
					...
					var/new_galleryimg_nsfw = trim(tgui_input_text(user, "Input the image link (https, hosts: gyazo, lensdump, imgbox, catbox):", "NSFW Gallery Image", max_length = MAX_MESSAGE_LEN, encode = FALSE), MAX_MESSAGE_LEN) //OV EDIT - Discord Removed
				...
				if("ooc_extra")
					...
					var/new_extra_link = trim(tgui_input_text(user, "Input the accessory link (https, hosts: catbox):", "Song URL", ooc_extra, max_length = MAX_MESSAGE_LEN, encode = FALSE), MAX_MESSAGE_LEN) //OV EDIT - Discord Removed
